...
const KepegawaianPage = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      setMsg('Gagal menyimpan data kepegawaian.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-base font-bold text-slate-800">Admin Kepegawaian (HRD)</h1>
            <p className="text-xs text-slate-500">
              Data Induk Pegawai, Riwayat Jabatan/SK, Pengajuan Cuti/Izin & Approval Berjenjang (K.TU &rarr; Kepsek &rarr; Keuangan)
            </p>
          </div>
        </div>

        <div className="flex items-center space-x-2">
          {activeTab === 'pegawai' && (role === 'kepegawaian' || role === 'admin' || role === 'kepala_tu') && (
            <button
          ...
          {activeTab === 'riwayat' && (role === 'kepegawaian' || role === 'admin' || role === 'kepala_tu') && (
            <button
          ...
          {activeTab === 'kinerja' && (role === 'kepegawaian' || role === 'admin' || role === 'kepala_sekolah' || role === 'kepala_tu') && (
            <button
              onClick={() => handleOpenModal('add_kinerja')}
              className="flex items-center space-x-1.5 px-3.5 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-xs font-semibold shadow transition-colors"
            >
              <Plus className="w-4 h-4" />
      ...
      {/* Sensitive Data Info Banner for Guru */}
      {(role === 'guru' || role === 'siswa') && (
        <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-900 text-xs flex items-center space-x-2">
          ...
          <span>Data Kepegawaian bersifat sensitif. Anda melihat profil & riwayat kepegawaian pribadi Anda sendiri.</span>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-1.5 flex flex-wrap gap-1">
        <button
          ...
          className={`flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-semibold transition-colors ${
            activeTab === 'pegawai' ? 'bg-emerald-700 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
          <UserCheck className="w-4 h-4" />
          <span>1. Data Induk & Gaji Pegawai</span>
        </button>

        <button
          onClick={() => setActiveTab('pengajuan')}
          className={`flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-semibold transition-colors ${
            activeTab === 'pengajuan' ? 'bg-emerald-700 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
          <CheckCheck className="w-4 h-4" />
          <span>2. Pengajuan Cuti & Approval SK</span>
        </button>

        <button
          onClick={() => setActiveTab('riwayat')}
          className={`flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-semibold transition-colors ${
            activeTab === 'riwayat' ? 'bg-emerald-700 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
          <Briefcase className="w-4 h-4" />
          <span>3. Riwayat Jabatan & SK</span>
        </button>

        <button
          onClick={() => setActiveTab('kinerja')}
          className={`flex items-center space-x-2 px-3.5 py-2 rounded-lg text-xs font-semibold transition-colors ${
            activeTab === 'kinerja' ? 'bg-emerald-700 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
          <Award className="w-4 h-4" />
          <span>4. Penilaian Kinerja Pegawai</span>
        </button>
      </div>
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {filteredPegawai.map((p) => (
                        <tr key={p.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 font-semibold text-slate-700">{p.jabatan || 'Guru'}</td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3 text-center">
                            {(role === 'kepegawaian' || role === 'admin' || role === 'kepala_tu') && (
                              <button
                                onClick={() => handleOpenModal('edit_pegawai', p)}
                                className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded text-[11px] font-semibold transition-colors shadow-sm"
                              >
                                Edit
                              </button>
                            )}
                    ...
                    <tbody className="divide-y divide-slate-200">
                      {pengajuanList.map((pg, idx) => (
                        <tr key={pg.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-center">
                            {(role === 'kepala_tu' || role === 'kepala_sekolah' || role === 'admin' || role === 'kepegawaian') && (
                              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-4">
            <h2 className="text-sm font-bold text-slate-800 border-b pb-2">
              ...
              {modalType === 'edit_pegawai' && 'Edit Data Kepegawaian (NIP, SK, Gaji & Status)'}
                ...
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <input
                        type="text"
                        placeholder="Isi NIP resmi pegawai..."
                        value={formData.nip || ''}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block font-semibold text-slate-700 mb-1">Jabatan / Unit</label>
                      <input
                        type="text"
                        required
                        placeholder="Guru / Staf TU"
