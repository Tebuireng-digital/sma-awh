...
import { MapPin, Clock, BookOpen, CheckCircle, AlertCircle, Sparkles, Send } from 'lucide-react';
...
const GuruPresensi = () => {
  ...
  const handlePresensiMasuk = async () => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: 'Presensi masuk KBM berhasil dicatat!' });
    } catch (e) {
  ...
  const handleSimpanJurnal = async (e) => {
    e.preventDefault();
    if (!presensiActive) {
      setMessage({ type: 'error', text: 'Anda harus melakukan Presensi Masuk terlebih dahulu!' });
      return;
    }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card-surface p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
            1. Jadwal Mengajar & Presensi GPS
          </h2>
        ...
        <div className="space-y-3">
          <label className="block text-xs font-medium text-slate-700">Pilih Jam KBM Hari Ini:</label>
          ...
          <div className="pt-2">
            <button
              ...
              className="w-full py-2.5 bg-emerald-800 hover:bg-emerald-900 text-white font-semibold text-xs rounded-md shadow-sm transition-colors flex items-center justify-center space-x-1.5"
            >
              <CheckCircle className="w-4 h-4" />
              <span>{submitting ? 'Memproses Presensi...' : 'Presensi Masuk KBM (Geofence)'}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="card-surface p-5 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">
            2. Jurnal Mengajar & Target Promes
          </h2>
        ...
        <form onSubmit={handleSimpanJurnal} className="space-y-3">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-2.5 bg-indigo-900 hover:bg-indigo-950 text-white font-semibold text-xs rounded shadow-sm transition-colors flex items-center justify-center space-x-1.5"
          >
            <Send className="w-3.5 h-3.5" />
            <span>Simpan Jurnal & Update Promes</span>
          </button>
        </form>
      </div>
    </div>
