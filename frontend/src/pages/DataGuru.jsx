...
import { Users, Plus, Search, Edit3, Trash2, CheckCircle, AlertCircle, RefreshCw, X, KeyRound, Layers } from 'lucide-react';
...
const DataGuru = () => {
  ...
  const openEditModal = (guru) => {
    setEditingGuru(guru);
    let roles = [];
    if (guru.additional_roles) {
      roles = Array.isArray(guru.additional_roles)
        ? [...guru.additional_roles]
        : (typeof guru.additional_roles === 'string' ? JSON.parse(guru.additional_roles || '[]') : []);
    }
    setFormData({
    ...
    <div className="space-y-6">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto border border-slate-200">
            <div className="px-5 py-4 bg-emerald-800 text-white flex items-center justify-between sticky top-0 z-10">
              <h3 className="text-sm font-bold tracking-tight">
                {editingGuru ? 'Edit Data Guru & Merangkap Jabatan' : 'Tambah Data Guru Baru'}
              </h3>
              <button onClick={() => setShowModal(false)} className="text-emerald-200 hover:text-white cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="p-5 space-y-3.5">
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg space-y-2 mt-2">
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed">
                  Centang tugas rangkap guru ini. Guru dapat mengakses fitur & modul tersebut menggunakan akun login miliknya tanpa ganti akun.
                </p>
              ...
              {!editingGuru && (
                <div className="p-2.5 bg-emerald-50 border border-emerald-200 rounded text-[11px] text-emerald-900 font-mono">
                  💡 Akun user login otomatis dibuat: <strong>guru_{formData.id_guru || 'ID'}</strong> (Password default: <code>guru123</code>). Data otomatis tersinkron ke Kepegawaian.
                </div>
              )}
              ...
              <div className="pt-3 flex justify-end space-x-2 border-t border-slate-100">
                <button
                  ...
                  className="px-3.5 py-2 bg-slate-100 text-slate-700 rounded text-xs font-semibold hover:bg-slate-200 cursor-pointer"
                >
                  Batal
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-4 py-2 bg-emerald-800 text-white rounded text-xs font-semibold hover:bg-emerald-900 shadow-sm cursor-pointer"
                >
